...
public class SwerveModule {
	...
	public SwerveModule(Constants.Chassis.SwerveModuleConfiguration configs) {
		...
		steerMotorConfig.Feedback.RotorToSensorRatio = Constants.Chassis.MK4I.kAngleGearRatio;
	...
	public SwerveModulePosition getPosition() {
		...
		double drivePosition = driveRot - steerRot * Constants.Chassis.MK4I.kCouplingRatio;
		return new SwerveModulePosition(
				Conversions.falconToMeters(
						drivePosition,
						Constants.Chassis.MK4I.kWheelCircumference,
						Constants.Chassis.MK4I.kDriveGearRatio),
	...
	public void setDesiredState(SwerveModuleState desiredState, boolean closedLoop) {
		...
		double targetVelocity = Conversions.MPSToFalcon(desiredState.speedMetersPerSecond,
				Constants.Chassis.MK4I.kWheelCircumference, Constants.Chassis.MK4I.kDriveGearRatio) * cosineScalar;

		if (closedLoop) {
			driveMotor.setControl(closedLoopDriveRequest.withVelocity(targetVelocity));
		} else {
			double voltage = Conversions.falconToMPS(targetVelocity, Constants.Chassis.MK4I.kWheelCircumference,
			Constants.Chassis.MK4I.kDriveGearRatio) / Constants.Chassis.kMaxSpeed * 12.0;
